...
test("Restricted LLM Provider should not appear for unauthorized users", async ({
  ...
  try {
    groupId = await createUserGroup(page, restrictedGroupName);
    ...
    providerId = await createRestrictedProvider(
      page,
    ...
    if (providerId) {
      await deleteProvider(page, providerId);
      console.log(`Deleted provider with ID: ${providerId}`);
    }

    if (groupId) {
      await deleteUserGroup(page, groupId);
